...
Indices: on the continuous side, $i$ indexes collocation / sample
points, $j$ components, and $k$ time steps. On the discrete grid the
same letters are reused: $i, j$ run over $(x, y)$ and $n$ over $t$
(e.g. $u_i^n$ for the value at grid cell $i$, time step $n$).
