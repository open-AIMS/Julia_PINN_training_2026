...
# Autodiff time derivative through the network.
# Use Zygote for the INNER derivative too (reverse-over-reverse for a
# scalar-input scalar-output composition is well-supported in current
# Lux/Zygote). Composing ForwardDiff inside Zygote breaks here because
# Lux's matmul rrule returns a Tangent{Any, ...} wrapping
# ForwardDiff.Partials that ChainRules.OneElement can't construct an
# indexing pullback for — a known Lux+Zygote+ForwardDiff compatibility
# gap as of mid-2026.
function dxdt(t::F, ps, st)
    g = Zygote.gradient(τ -> x_θ(τ, ps, st), t)[1]
    g === nothing ? zero(F) : g
end
