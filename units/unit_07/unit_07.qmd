...
::: {.exercise-hint-text}
The cheapest reference isn't FD at all — d'Alembert gives $\eta(x,t) = \tfrac12\eta_0(x-ct) + \tfrac12\eta_0(x+ct)$ with $c=\sqrt{gH} \approx 9.9$ m/s, so at the half-time snapshot the initial bump has split into two half-height copies. This is a two-network system, so `phi = discretization.phi` is a *vector* — use `ηf(x,t) = first(phi[1]([x,t], res.u))` and `uf(x,t) = first(phi[2]([x,t], res.u))`. The residual is first-order, so each term is a *single* `ForwardDiff.derivative` (not nested): `ForwardDiff.derivative(τ->ηf(x,τ), t) + 10*ForwardDiff.derivative(ξ->uf(ξ,t), x)`. Bin $t$ into ~10 ranges and plot the per-bin mean. The snapshot exposes spectral bias (one lump not two) + loss imbalance; the non-decreasing histogram exposes causal violation.
:::
:::
