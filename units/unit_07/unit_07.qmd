...
Workflow 3 has a recurring shape, independent of the physics — worth
seeing once in the abstract, because every source-recovery PINN (the
Unit 1 river-mouth $\psi(t)$, the capstone storm envelope, a spatial
source $g(\mathbf{x})$, a closure law) is an instance of it. Write the
forward problem so the unknown enters linearly through a *known* shape,

$$\partial_t u = \mathcal{L}u + s(\mathbf{x})\,f(t),$$

with $\mathcal{L}$ the known spatial operator, $s$ a known source
*shape*, and $f$ the unknown *driver* you want back. Three structural
choices define the paradigm:

- **Two networks, trained jointly.** A field network
  $u_\theta(\mathbf{x}, t)$ and a *small* auxiliary network $f_\phi$ for
  the unknown function — optimised together in one loss, not in two
  stages.
- **Hard-wire the IC and BCs into the field ansatz** (§7.3), so the only
  soft terms left are the residual and the data misfit (plus any flux
  BC). Fewer terms is fewer ways for the optimiser to cheat.
- **The driver enters *only* the residual.** Rearranged, the residual
  pins $f_\phi \approx (\partial_t u_\theta - \mathcal{L}u_\theta)/s$:
  the recovered driver is read off *derivatives of the field network*.
  Source recovery is deconvolution wearing a PINN costume.

That last fact is the whole difficulty, and the sharpest amendment the
recipe needs. The forward operator *smooths* the driver (diffusion is a
low-pass filter), so a field that matches the data *in value* can have
derivatives — and therefore a recovered peak — that collapse far below
the truth. **The cure is not finer collocation; it is the loss
weights.** Make the data term dominate at the operating point
($\lambda_d \gg \lambda_r$, reached by ramping up from the gentle start
of step 3 above): forcing $u_\theta$ to honour sharp data keeps its
derivatives sharp and lifts the recovered peak. Add only a *whisper* of
smoothness on the driver, $\lambda_\text{reg}\int|f_\phi'|^2$ — enough to
kill sample-to-sample ringing, far too small to flatten the signal. Too
