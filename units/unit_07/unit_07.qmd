...
![Left: the closed-form three-lobed harmonic $u(r, \theta) = r^3 \sin(3\theta)$ on the unit disk, with Dirichlet boundary $u(R, \theta) = \sin(3\theta)$ and a small inner cutoff $r_{\min}$. Right: a schematic PINN collocation cloud — the actual solve samples ~400 random interior residual points (green) and ~36 uniform boundary points (orange); the diagram draws fewer for clarity. The mesh-free advantage: random sampling on a disk needs no quadrature scheme or curvilinear mesh.](figures/laplace_disk.svg){#fig-laplace-disk width=92%}
