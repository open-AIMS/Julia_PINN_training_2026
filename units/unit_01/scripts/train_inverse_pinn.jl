...
# which collapses to a single 2-D wave equation for the surface elevation
#     ∂_tt η = ∇·(g H ∇η).
# Inverse problem: recover ψ(t) := η(x_r, y_r, t), the time-history at the
# river-mouth source cell, given η_obs(t_k) at four gauge sites.
#
# Architecture. One MLP η_θ : (x̃, ỹ, t̃) → η, with inputs scaled to
# [−1, 1]. ψ_θ(t) is *defined* as the network evaluated at (x_r, y_r, t).
# No separate ψ network — keeps Unit 1 conceptually clean.
#
# Loss = λ_d  · gauge-data mismatch
#      + λ_p  · PDE residual at random water collocation points
#      + λ_ic · zero IC at t = 0 (η and ∂_t η)
#
# Residuals are computed by finite differences in (x, y, t) rather than
# Zygote-of-Zygote — Unit 1 is a teaser, the full autodiff PINN is in
# Units 5–9. FD stencils are O(h²) and chosen so each shift is large
# enough to be informative but small relative to the surge length scale.
...
function eval_loss(θ)
    ...
    return total_loss(θ, Xphys, Kx, Ky, Xz, Xt, Xsm)
end
...
function train_adam!(θ0)
    ...
    for step in 1:N_STEPS
        ...
        g = first(Zygote.gradient(
            θ -> first(total_loss(θ, Xphys, Kx, Ky, Xz, Xt, Xsm)),
            θ))
        ...
        if step % log_every == 0 || step == 1
            l, parts = total_loss(θ, Xphys, Kx, Ky, Xz, Xt, Xsm)
            @printf("  step %4d   loss = %.4e   (data=%.3e phys=%.3e ic=%.3e tic=%.3e sm=%.3e)   %.1fs\n",
                    step, l, parts..., time() - tic)
        end
    end
...
CSV.write(joinpath(DATA_DIR, "psi_recovered.csv"),
...
let df = DataFrame(t = psi_truth.t)
    ...
    CSV.write(joinpath(DATA_DIR, "gauges_pinn_pred.csv"), df)
end
